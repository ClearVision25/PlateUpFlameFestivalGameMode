    ...
    public class FireScoreView : UpdatableObjectView<FireScoreView.ViewData>
    {
        public class UpdateView : IncrementalViewSystemBase<ViewData>, IModSystem
        {
            ...
            protected override void OnUpdate()
            {
                ...
                int date = fireScore.Date;
                if (hasSFireScore)
                {
                    switch (fireScore.GameMode)
                    {
                        case FireGameMode.FireSurvival:
                            score = fireScore.Score;
                            target = fireScore.Target; 
                            date = fireScore.Date;
                            break;
                        case FireGameMode.CoinCollection:
                            score = fireScore.Score;
        ...
        private FireGameMode gameMode = FireGameMode.CoinCollection;
        ...
        void Update()
        {
            ...
            if (gameMode == FireGameMode.FireSurvival)
            {
                ...
                if (FireMultiplier)
                {
                    ...
                    FireMultiplier.color = Color.white;
                }
                if (Target)
                {
            ...
            else if (gameMode == FireGameMode.CoinCollection)
            {
                if (Score)
                {
                ...
                if (Target)
                {
                    ...
                    { 
                        string text = $"Target: <sprite name=\"{spriteName}\"> {target} by End of Day {date}";
                        Target.text = string.Join("\n", text);
                        Target.color = Color.white;
                    }
                }
